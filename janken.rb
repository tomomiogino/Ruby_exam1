...
class Player
  def hand
    ...
    while true
      ...
      true_number = ["0", "1", "2"]
      if true_number.include?(input_hand)
        # 「input_hand」(取得した値)をそのまま返します。ヒント！ 「return」を使用します。
...
class Janken
  def pon(player_hand, enemy_hand)
    ...
    # ジャンケンの勝ち負けの判定をする変数（judge）を用意
    judge = (player_hand - enemy_hand + 3) % 3
    # 以降の型の例に沿って実装しましょう ※実装する時場合はコメントアウトに注意しましょう
    # あいこの場合（Playerクラスの戻り値(player_hand)とEnemyクラスの戻り値(enemy_hand)の値が同じだった場合）
    if judge == 0
      puts "あいこ"  # 「あいこ」を出力します。
      #「true」を返してじゃんけんを繰り返し実行させます。ヒント！ 「return」を使用します。
      return true
    # もしもプレイヤー(自分)が勝ちの場合
    elsif judge == 2
